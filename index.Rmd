...
We want to collapse all of the values for the emission data across the different individual year variables into one new `Emissions` variable. We will identify what year they are from by creating a new `Year` variable. The `cols =` argument allows us to specify which columns we want to use or not use to pull from to create these new columns. We want to keep our `country` data so we will exclude it using the `-` sign, by default all other columns will be used.
...
First, the `Date` column looks a bit strange. The format of the numbers look like the year followed by the number 12.
...
We can also do the same thing by using the `reduce()` function of the `purrr` package. 
