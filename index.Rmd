...
```{r, echo = FALSE, out.width="400px"}
knitr::include_graphics('https://www.epa.gov/sites/production/files/styles/medium/public/2020-04/sources-of-greenhouse-gas-emissions-in-2018.jpg')
```
...
See [here](https://www.epa.gov/report-environment/greenhouse-gases){target="_blank"}  [here](https://nca2014.globalchange.gov/){target="_blank"}, and [here](https://world101.cfr.org/global-era-issues/climate-change/climate-change-adaptations){target="_blank"}  for more information about how greehouse gases have influenced global temperatures.
...
2. Merging data with different time spans
3. How to create effective longitudinal data visualizations with `ggplot2`
4. How to add labels to `ggplot2` plots

Statistical methods:

1. Define and recognize the difference between populations and samples # avocado comment - I think this may have gotten deleated - this is also why I used cor instead of cor.test -if you think that these are indeed samples... then maybe we should not supress the gray lines for the loess trend lines
2. Basic understanding of correlation 
3. Difference between correlation and causation
3. Difference between correlation and linear regression association
...
AVOCADO: Unclear purpose of text above? I know the definition of GWP is in the text, but can we remove the text and just state the definition? 
Avocado comment: I thought that the explanation of how gases actually conrtibute to climate change by trapping heat was really useful for introducing what GWP is. I reduced and separted this a bit, but feel free to change as you see fit.
...
As CO2 levels rize in the ocean, the pH becomes more acidic, which makes it difficult for organisms to maintain their shells or skeletons that are made of calcium carbonate, thus making it more difficult for these organisms to survive and impacting their role in the ecosystem and food chain. 
...
avocado comment: I added this to the learning objectives... do you want to also move this somewhere in the analysis section or keep this here?
...
However, for these files there are some lines that we would like to not import because the number of columns differ for some rows. If we dont account for this, then we may end up importing fewer columns of the data that we would like.
...
Right now our data is in what is called **"wide"** format. In wide format, each varaible is listed as its own column. In contrast, in long format, variables maybe collapsed into a column that identifies the variables and a column of values. See [here](https://en.wikipedia.org/wiki/Wide_and_narrow_data){target="_blank"} for more information about the difference bewteen the two formats.
...
(AVOCADO: I think there is a formal definition for this -- e.g. one row per observation etc?)
avocado comment: is this better?
...
You may also note that the `Year` variable is currently of class type character. We would like to change it to be numeric. To do this we will use the `mutate()` function, which is also part of the `dplyr` package. This function allows us to create and modify variables.
...
First we will use the `bind_rows()` function which automatically creates `NA` values for any variable that is missing from a data object that is added by combining the data object with another that contains that missing varaible using this function.
```{r}
data_wide_br <- 
  list(CO2_emissions, gdp_growth, energy_use) %>% 
  bind_rows()
...
This is something to keep in mind when you are wranglign your data. The choice of what function to use and how should depend on how you want the data to be after you combine the different sources of data together.
...
Because our data set contains other variables, we first need to filter our data to only include the CO2 emissions data by using the `filter()` function of the `dplyr` package. 
...
To change title, caption, changing the axis labels, we can use the `labs()` function. 
...
Next, we go back to using our `data_long` data set. 
...
avocado comment: I made this section hopefully more clear about how to use `fct_reorder()`
...
To use this function, the variable that is to be reordered is listed first. The variable that is that is being used to determine the order is listed second. Finally a function to apply to the variable listed second is listed third. This function is used to determine the order.  In this case we want to determine the last value of the `Value` variable using the `last()` function (recall that this is also a function of the `dplyr` package). Then the `Country` variable will be ordered by the last value of the `Value` variable.
...
Now, we will visualize all the variables in our data set.
...
First, we will filter out the data about disasters and temperature as this is only for the US, by using the `filter()` function and `!=` which indicates "not equal to".
...
We know that the data sets do not span the same amount of time. 
...
We can use the [correlation coefficient](https://rafalab.github.io/dsbook/regression.html). 
...
Thinking about father's and son's heights -- note that if the correlation is positive and lower than 1, our prediction is closer, in standard units, to the average height than the value used to predict, $x$, is to the average of the $x$s. 
This is why we call it _regression_: the son regresses to the average height. In fact, the title of Galton's paper was: _Regression toward mediocrity in hereditary stature_.
...
Try creating the similar plots, but comparing CO2 emissions with the other variables and add a linear regression line. 
...
(CO2_world | Top10t) / (CO2_temp_US_facet | CO2_temp_US)+
  plot_layout( widths = c(4, 6), heights = c(20, 20))
  # plot_layout(widths = c(2,4),
  #             heights = unit(c(4 , 4), c('cm', 'cm')))

library(cowplot)

plot_grid(CO2_world, Top10t, CO2_temp_US_facet, CO2_temp_US, ncol = 2, rel_widths=c(0.8,.8),rel_heights=c(0.9,0.8))


#reduce margins
margins_left <- c(0.4,0.2,0.4,0.2)
margins_right <- c(0.4,0.4,0.4,0.4)

CO2_world <- CO2_world + theme(plot.margin=unit(margins_left, "cm"))
Top10t <- Top10t + theme(plot.margin=unit(margins_left, "cm"))
CO2_temp_US_facet <- CO2_temp_US_facet+ theme(plot.margin=unit(margins_right, "cm"))
CO2_temp_US <- CO2_temp_US + theme(plot.margin=unit(margins_right, "cm"))
...
```{r, echo = FALSE}
png(here::here("img", "mainplot.png"), width = 800, height = 700)
(CO2_world | Top10t) / (CO2_temp_US_facet | CO2_temp_US) +
