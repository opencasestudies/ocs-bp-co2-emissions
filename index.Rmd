...
Let's briefly discuss the relationship between correlation and linear regression, which is further described in [Introduction to Data Science book](https://rafalab.github.io/dsbook/regression.html). 
...
Re-organizing the terms so that $Y$ is on the left side and everything else is on the right side:
...
In our example, we can calculate the slope and intercept using the formula above. 
```{r}
wide_US_summary <-
  wide_US %>%
