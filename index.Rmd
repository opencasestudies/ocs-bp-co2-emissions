...
Indeed, it does look like there is a positive trend. 


#### {.question_block}
<b><u> Question Opportunity </u></b>

- Make similar plots for between CO2 emissions and other variables. Are there other pairs variables that look like they might have a similar pattern to CO2 emissions? Does this match what we saw above? 
...
Now we will create a plot that summarizes our major findings. We will use the `plot_layout()` function of the `patchwork` R package.

```{r, out.width= "9000px", "out.height= 7000px"}
# (CO2_world | Top10t) / (CO2_temp_US_facet | CO2_temp_US)
(CO2_world | Top10t) / (CO2_temp_US_facet | CO2_temp_US) +
  plot_layout(widths = unit(c(9, 9), c('in', 'in')),
              heights = unit(c(2, 4), c('in', 'in')))
```

```{r, eval = FALSE}
png(here::here("img", "mainplot.png"), width = 800, height = 700)
(CO2_world | Top10t) / (CO2_temp_US_facet | CO2_temp_US) +
  plot_layout(widths = unit(c(10, 10), c('in', 'in')),
              heights = unit(c(2, 4), c('in', 'in')))
dev.off()
```
