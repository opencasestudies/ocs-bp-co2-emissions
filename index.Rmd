...
mortality     <- readxl::read_xls(here("data/API_SP.DYN.CDRT.IN_DS2_en_excel_v2_804384.xls"))
```
...
Finally, we will read in the `data/mortality.csv` file containing mortality data. 

```{r}
mortality <- readr::read_csv(here("data/mortality.csv"), skip = 3)
```
...
We will need to do some data wrangling to use to allow us to evaluate how CO2 emissions have changed over time and how emissions may relate to energy use, mortality, GDP etc.
...
Next, we look at the crude mortality rate (`mortality`) data.
...
AVOCADO: there is a column called `X65`. Unclear if I messed something up? 

We see there are `r nrow(mortality)` rows and `r ncol(mortality)` columns. 

That is looking better! However, we also want to remove some variables like: `Country Code`, `Indicator Name`, and `Indicator Code`. 
We can do that using the `select()` function of the `dplyr` package. 
We can use the minus operator `-` to indicate what variables we do not want to keep. 
Otherwise, we will perform similar modifications as we performed on the other data sets. 
Note that these variable names need quotation marks around them because they have spaces. 
...
OK, in this case it looks like there are some regions included as well such as `"East Asia & Pacific (excluding high income)"`.

Unfortunately, we do not know of a less manual way of doing this, so we do this in the following way: 
...
AVOCADO: could we do a left join with this data set below so that these regions don't get joined? instead of the code chunk above? 
...
We primarily use the `full_join()` function as we have different time spans for each data set and we would like to retain as much data as possible. 
The`full_join()` function will simply create `NA` values for any of the years that are not in one of the data sets. 

However, for the `mortality` data, we will use `left_join()` so that the extra regions in the `mortality` that we don't want (e.g. `"East Asia & Pacific (excluding high income)"`) will be excluded. 
...
data_wide <- CO2_emissions %>%
  ...
  left_join(mortality, by = c("Country", "Year", "Label"))
...
data_wide <- 
  list(data_wide, mortality) %>% 
  reduce(left_join, by = c("Country", "Year", "Label"))
...
If we want to really look at how two indicators may relate to each other, it is important that both data sets span the same amount of time. Therefore, we will limit this plot to only the years where the data overlaps for both CO2 emissions and temperature.
