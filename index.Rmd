...
We need to use the `.` notation to indicate that we are using the data that we already used as input (on the right side of the pipe) to our `mutate()` function (on the left side of our pipe), which in this case is the entire `us_disaster` tibble for our `select()` function. The the output from the `select()` function will be used for the `rowSums()` function. 
