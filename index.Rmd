...
remotes::install_github("tdhock/directlabels", type = "source", dependencies = TRUE)
...
##### [[source]](https://web.archive.org/web/20250323011220/https://web.archive.org/web/20250323011220/https://data.globalchange.gov/report/nca3-overview){target="_blank"}
...
The [National Climate Assessment Report](https://web.archive.org/web/20250323011220/https://data.globalchange.gov/report/nca3-overview){target="_blank"} states that:
...
2) The National Climate Assessment (NCA) [summary from 2014](https://web.archive.org/web/20250101083212/https://nca2014.globalchange.gov/){target="_blank"}) 
3) The [Global Center on Adaptation website](https://gca.org/12-great-examples-of-how-countries-are-adapting-to-climate-change/){target="_blank"} about how countries are adapting to climate change
...
We will begin by loading the packages that we will need. If you haven't yet installed any of the packages, you will need to do that first:
...
[`directlabels`](https://cran.r-project.org/web/packages/directlabels/refman/directlabels.html){target="_blank"} | to add labels to plots easily
...
These emissions are one of the contributing factors to rising global temperatures, which can have a great influence on [public health](https://www.epa.gov/report-environment/greenhouse-gases){target="_blank"}  as illustrated in the following image:
...
Due to a process called [thermal inertia](https://en.wikipedia.org/wiki/Volumetric_heat_capacity#Thermal_inertia), the heat stored in the ocean will eventually be transferred to the surface of the Earth long after the gases were emitted that resulted in the increased ocean temperature.
...
##### [[source]](https://web.archive.org/web/20250323011220/https://data.globalchange.gov/report/nca3-overview){target="_blank"}
...
Furthermore, greenhouse gas emissions are believed to influence weather patterns as shown in this [report](https://web.archive.org/web/20250323011220/https://data.globalchange.gov/report/nca3-overview){target="_blank"}. 
...
```{r, eval=FALSE}
# install.packages("OCSdata")
# You will need to install the package if you haven't already done so.
library(OCSdata)
...
load(here::here("OCS_data", "data", "imported", "co2_data_imported.rda"))
```
...
In this next code chunk, we will introduce the `%>%` operator from the `dplyr` package. 
This allows us to use our `CO2_emissions` data and modify it sequentially using  multiple steps. We will start with just one step however, but we will see the utility of this soon where we will have multiple steps each having a %>% at the end into the next step. We can think of this as a "take this data and then" statement". So in this case. Take the CO2_emissions data and then pivot it into the long format. 
...
Note that here we are now using multiple steps with our `%>%` pipe operator. So the CO2_emissions data is used to rename country to Country and then the out put of this is taken to be mutated to have Year as a numeric variable .
...
```{r}
direct.label(Top10b, method = list("last.points")) +
...
[Here](https://cran.r-project.org/web/packages/directlabels/refman/directlabels.html){target="_blank"} is a list of options.
...
See [here](https://medium.com/data-science/estimating-non-linear-correlation-in-r-62c6571cb1db){target="_blank"} for more information on nonlinear relationships.
...
If you want to learn more about why this is the calculation to determine the strength of the relationship between two variables, see this [link](https://mac-stat.github.io/Stat155Notes/discrete-random-variables.html). 
...
[National Climate Assessment Report](https://web.archive.org/web/20250323011220/https://data.globalchange.gov/report/nca3-overview){target="_blank"}    
[Greenhouse gases](https://www.epa.gov/report-environment/greenhouse-gases){target="_blank"} 
[Climate change](https://gca.org/){target="_blank"}
...
We would like to acknowledge [Qier Meng](https://scholar.google.com/citations?user=EVGygAwAAAAJ&hl=en) and [Michael Breshock](https://mbreshock.github.io/) for their contributions to this case study. 
