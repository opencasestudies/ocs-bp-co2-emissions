...
We can also replace all instances of `"-99"` with `NA` using the `na = ` argument of the `read_csv()` function.
We will do so as: `na = "-99"`. The "-99" needs to be in quotation marks because this argument expects characters.
...
```{r}
mortality <- readr::read_csv(here("data/mortality.csv"), skip = 5)
```
...
We can see that we have a large [tibble](https://tibble.tidyverse.org/). 
A tibble is the `tidyverse` version of a data frame. 
It is essentially a table with variable information arranged as columns, and individual observations arranged as rows. 
We can see that the tibble gives us information about the class of each variable. 

For example the `country` variable is made up of character (abbreviated as `chr`) values.
We see that we have 265 different country variables and CO2 emission values for 192 different years (from 1751 to 2014). 
Recall that the values are emissions in metric tons also called tonnes. 
We can also see that there are fewer `NA` values for later years.
...
Ok, let's modify `CO2_emissions` to make it more usable for making visualizations. 
...
We also want to rename the `country` variable to be capitalized.
We can use the `rename()` function of the `dplyr` package to rename this variable. When renaming variables the new name is listed first before the `=`. We will also modify the `Emissions` data by dividing it by 1000 to make the numbers smaller. To do this we will use the `mutate()` function, which is also part of the `dplyr()` package. This function allows us to create and modify variables. You may also note that the `Year` variable is currently of class type character. We would like to change it to be numeric. This can also be accomplished using the `mutate()` function.
