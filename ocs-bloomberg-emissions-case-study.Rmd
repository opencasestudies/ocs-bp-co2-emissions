...
Now let's take a look at Disaster data for the US.
...
We can see that the rate of disasters appears to be increasing over time.
...
This looks a bit awkward, because the each type of data spans a different time spans.
...
df_long %>%
  ...
  theme(axis.text.x = element_text(size = 12,angle = 90),
       ...
       plot.caption = element_text(size = 12),
         plot.title = element_text(size = 16))
```
It looks like the overlapping time spans for the different datasets is also from 1980 to 2014.
...
If we want to really look at how two indicators may relate to eachother it is imporant that both datasets span the same amount of time.
...
df_wide_US <-df_long_us %>%
  select(-c(Label, Anomaly, Anomaly_color, Mean, "Change (%)")) %>%
...
As natural disaster rates rise, countries are experiencing higher rates of death due to natural disasters https://ourworldindata.org/natural-disasters
