...
To remove entries for countries with NA values we can use `filter(complete.cases(.))`.
```{r}
...
Great! We've created our first plot. We can see that many countries show a dramatic increase in emissions over time with a handful of countries with particularly high levels.
...
Since we will be ploting only one value each year, we no longer need to assign a `group` in the `aes()` argument. this time we will make the size of the line that will be plotted a bit larger using the `size()` argument in the `geom_line()` function.
...
```{r}
df_long %>%
  filter(Indicator == "Deaths", Year == 1980) %>% arrange(-Value)

df_long %>%
  filter(Indicator == "Deaths", Year == 1995) %>% arrange(-Value)

df_long %>%
   filter(Indicator == "Deaths", Year == 2017) %>% arrange(-Value)
```
