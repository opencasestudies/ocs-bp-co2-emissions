...
## Motivation
This [report](https://www.epa.gov/report-environment/greenhouse-gases){target="_blank"} provides information about what greenhouse gases are and how they influence global temperatures. It states that

> "beginning in the late 1700s, the net global effect of human activities has been a continual increase in greenhouse gas concentrations"
...
Another option is to create a binary outcome of if optimal consumption was achieved or not.
...
How do different age groups look:
...
Under<-diet_and_guidelines %>%
  ...
  ggplot(aes(y = Relative_Percent, x = food, color = sex)) +
  geom_point(position=position_jitterdodge(), 
             color = "dark grey",
             alpha = 3/10,
             aes(fill = sex))+ # this specifies how to separate the points
  geom_boxplot(outlier.shape = NA, color = "black" , aes(fill = sex))+
  ylim(c(0,100))+
    scale_fill_manual(values = c("#481567FF", "#1F968BFF"))
Under
#scales:: show_col(viridis_pal()(3))
Under<-Under + geom_point(data = diet_and_guidelines %>%
                    filter(direction == "low",
         location_name == "United States"),
             position=position_jitterdodge(
               ...
               color = "black",
               size = 3,
             aes(fill = sex, shape = location_name))
...
labs(title="Underconsumed foods associated with health risk",
        x ="", y = "Percent consumption relative \n to guidelines")
...
#scales:: show_col(viridis_pal()(3))
Over<-Over + geom_point(data = diet_and_guidelines %>%
                    filter(direction == "high",
         location_name == "United States"),
             position=position_jitterdodge(
               jitter.width=0.01,
               ...
               size = 3,
             aes(fill = sex, shape = location_name))+
              ggforce::facet_zoom(ylim = c(0, 1500))
...
labs(title="Overconsumed foods associated with health risk",
        x ="", y = "Percent consumption relative \n to guidelines")
...
```{r}
cowplot::plot_grid(Over, Under, labels = c("A", "B"), ncol = 1)
